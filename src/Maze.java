...
public class Maze {
	...
	public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
		...
		Coordinate currentState = null;
		sc.push(start);

		while (!sc.isEmpty()) {
			currentState = sc.pop();

			if (isExit(currentState.getX(), currentState.getY()))
				return true;
				...
				getDirection(sc, currentState, visitedNodes);
			}
		}
	...
	private void getDirection(iSimpleContainer<Coordinate> sc, Coordinate c, iDeque<Coordinate> visitedNodes) {

		int x0 = c.getX();
		int y0 = c.getY();

		Coordinate right = new Coordinate(x0 + 1, y0);
		Coordinate left = new Coordinate(x0 - 1, y0);
		Coordinate up = new Coordinate(x0, y0 + 1);
		Coordinate down = new Coordinate(x0, y0 - 1);

		
		if (isInMaze(down) && !isVisited(down) && isAllowable(down))
			sc.push(down);
		
		
		if (isInMaze(right) && !isVisited(right) && isAllowable(right))
			sc.push(right);
		
		
		if (isInMaze(up) && !isVisited(up) && isAllowable(up))
			sc.push(up);
		
		if (isInMaze(left) && !isVisited(left) && isAllowable(left))
			sc.push(left);
	}
	...
	private boolean isVisited(Coordinate c) {
		return Character.compare(currentMaze[c.x][c.y], visited) == 0;
	}
	...
	private Coordinate getStartState(char[][] maze) {
		...
		for (int i = 0; i < rows; i++) {
			for (int k = 0; k < cols; k++) {
				if (currentMaze[i][k] == S) {
					start = new Coordinate(i, k);
					break;
				}
			}
		}
	...
	private boolean isAllowable(Coordinate c) {
		return currentMaze[c.x][c.y] == O || isExit(c.x, c.y);
	}
